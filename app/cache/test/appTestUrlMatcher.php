...
class appTestUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/events')) {
            // new_event
            if (0 === strpos($pathinfo, '/events/new') && preg_match('#^/events/new(?:\\.(?P<_format>json|xml|html))?$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
                    goto not_new_event;
                }

                return $this->mergeDefaults(array_replace($matches, array('_route' => 'new_event')), array (  '_controller' => 'WireDelta\\DemoBundle\\Controller\\EventsController::newEventAction',  '_format' => NULL,));
            }
            not_new_event:
